            ...
            if text.lower() == 'help':
                if sender in ban["owners"]:
                    ...
                    cl.sendMessage(to, str(helpM))
                     ...
                     cl.sendMessage(to,"您可以輸入wallpaper以獲得更多的桌布~")
            if sender not in ban["blacklist"]:
                if text.lower() in ['妹子圖','抽妹子'] :
                    cl.sendImage(to, "bot/linepy/wallpaper/{}-monmon.png".format(str(random.randint(0,int(settings["monmonpic"]-1)))))
                elif text.lower() in ['wallpaper','抽桌布'] :
                    cl.sendImage(to, "bot/linepy/wallpaper/{}-image.png".format(str(random.randint(0,int(settings["pic"]-1)))))
            if sender in ban["admin"] or sender in ban["owners"]:
                if text.lower() in ['speed','sp']:
                    cl.sendMessage(to,"機器人極緩慢的測速中...\n結果約為"+str(timeit.timeit('"-".join(str(n) for n in range(100))',number=1000)) + "秒")
                ...
                if text.lower() == 'restart':
                    ...
                    if msg.toType ==2:
                        ...
                        try:
                            ...
                            pass
                elif msg.text.lower().startswith("Add ") or msg.text.lower().startswith("add "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
                    if inkey not in ban["admin"] and inkey not in ban["blacklist"] and inkey not in ban["owners"]: 
                        ...
                        json.dump(ban, codecs.open('bot/ban.json','w','utf-8'), sort_keys=True, indent=4, ensure_ascii=False)
                elif msg.text.lower().startswith("Del ") or msg.text.lower().startswith("del "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
